...
import { DynamicCardsActions } from "../../redux/reducer/slice/home/dynamicCardsSlice";
import { DynamicSelector } from "../../redux/selectors/home/DynamicSelector";

export const HomeCard = ({ menuId, title }) => {
    const dynamicCardsData = useSelector(DynamicSelector);
    const dispatch = useDispatch();

    const cardState = dynamicCardsData?.cards?.[menuId] || { data: {}, isFetching: false };
    const rawData = cardState.data || {};
    const isFetching = cardState.isFetching;
    ...
    useEffect(() => {
        if (menuId) {
            dispatch(DynamicCardsActions.getAll({ menu_id: menuId }));
        }
    }, [menuId, dispatch]);
    ...
    return (
        <div className="w-full flex flex-col rounded-[10px] shadow-md relative">
            <div className="w-full p-4 bg-[#EDFAFE] rounded-t-[10px]">
                <span className="text-[#001F51] font-[700] text-[20px]">{title}</span>
            </div>
