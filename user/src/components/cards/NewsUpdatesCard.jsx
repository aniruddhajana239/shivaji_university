...
import { NewsSelector } from "../../redux/selectors/home/News";
import { NewsActions } from "../../redux/reducer/slice/home/news";

export const NewsUpdatesCard = ({ data }) => {
    ...
    useEffect(() => {
        if (data && data[2]?.id) {
            dispatch(NewsActions?.getAll({ menu_id: data[2]?.id }));
        }
    }, [data, dispatch]);

    const rawData = newsData?.data || {};
    const isFetching = newsData?.isFetching;
    ...
    return (
        <div className="w-full flex flex-col rounded-[10px] shadow-md relative">
            <div className="w-full p-4 bg-[#EDFAFE] rounded-t-[10px]">
                <span className="text-[#001F51] font-[700] text-[20px]">Updates / News</span>
            </div>
