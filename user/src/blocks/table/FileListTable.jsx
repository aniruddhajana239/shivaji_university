...
export const FileListTable = ({ title, content }) => {
    ...
    const transformedColumns = content?.sections?.columns?.map(column => {
        ...
        const columnMap = {
            ...
            "PDF": { heading: "PDF", accessor: "file_name" }
        };
                        ...
                        >
                            {transformedColumns?.map((column, colIndex) => {
                                ...
                                return accessor !== "file" ? (
                                    <td
                                        key={colIndex}
                                        className={`py-2 px-[19px] first:w-[150px]  ${accessor === "file_name"
                                            ? "last:w-[400px] md:last:w-full"
                                            : "w-[600px] md:w-[45%]"
                                            }`}
                                    >
                                        {accessor !== "file_name" ? (
                                            <span className="text-[14px] 2xl:text-[18px]">{cellValue ?? ""}</span>
                                        ) : (
